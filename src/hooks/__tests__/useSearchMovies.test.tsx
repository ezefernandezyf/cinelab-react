...
describe('useSearchMovies (mocked useApi)', () => {
  it('A - immediate option follows initialQuery (no immediate when empty, immediate when non-empty)', async () => {
    ...
    const mockUseApi = vi.fn().mockImplementation(
      (_fetcher: (signal?: AbortSignal) => Promise<unknown>, _options?: LocalUseApiOptions): UseApiReturn => {
        return { data: _options?.initialData ?? null, loading: false, error: null, refetch: async () => Promise.resolve(null) };
      }
    );
